...
def find_value(dictionary, book_id):
  book_found = ''
  for book in dictionary['catalog']['book']:
    if(book['id'] == book_id):
      book_found = book
...
with open('python/catalog.json') as catalog_json:
  ...
  print('Type: ' +  catalog_dict['catalog']['type'])
  print('Year: ' +  catalog_dict['catalog']['year'])
  ...
  for key, value in find_value(catalog_dict, 10).items():
    print(f'{key} : {value}')
